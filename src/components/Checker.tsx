...
export const Checker: Component = () => {
  ...
  const [limitTime, setLimitTime] = createSignal<string>(
    new Date(currentTime).toLocaleTimeString(),
  );
  const [soberTime, setSoberTime] = createSignal<string>(
    new Date(currentTime).toLocaleTimeString(),
  );
  ...
  function updateTimes() {
    return () => {
      ...
      if (count() == 0) {
        ...
        const currentTime = new Date().getTime();
        setLimitTime(
          new Date(currentTime + count() * 60 * 60 * 1000).toLocaleTimeString(
            [],
            options,
          ),
        );
